...
public:
  ...
  ServerBootstrap* group(
    ...
    if (acceptorFactory_) {
      ...
      // 否则就是用默认的
      workerFactory_ = std::make_shared<ServerWorkerPool>(
                         std::make_shared<ServerAcceptorFactory<Pipeline>>(acceptPipelineFactory_, childPipelineFactory_, accConfig_),
                         io_group.get(),
                         sockets_,// listening中的sockets
                         socketFactory_);
    }
  ...
  void bind(folly::AsyncServerSocket::UniquePtr s) {
    ...
    // 创建异步ServerSocket
    std::shared_ptr<folly::AsyncServerSocket> socket(s.release(), AsyncServerSocketFactory::ThreadSafeDestructor());
  ...
  void bindImpl(folly::SocketAddress& address) {
    ...
    // 牛逼！！！！，原来在这里，如果accept线程数大于1，那么久在所有的accept线程中重用端口进行监听
    ...
    // 定义一个lamda表达式，执行服务socket创建和accept操作，该函数一定会在accept线程中执行
    auto startupFunc = [&](std::shared_ptr<folly::Baton<>> barrier) {
      ...
      try {
        ...
        auto socket = socketFactory_->newSocket(address, socketConfig.acceptBacklog, reusePort, socketConfig);
        sock_lock.lock();
        // 添加到new_sockets中
